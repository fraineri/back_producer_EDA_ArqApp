...
controller.SubmitNewTest = async (req, res) => {
  try {
    ...
    if (dbTest) {
      return res
        .status(400)
        .send(`Test ${test.testId} already exists in the database`);
    }
...
controller.SubmitAnswer = async (req, res) => {
  try {
    ...
    console.log(
      `Submit answer: user = ${user}, test = ${testId}, question = ${questionId}, answer = ${answerId}`
    );
    ...
    if (question.answerID != "-1") {
      return res
        .status(400)
        .send(
          `answer already submited to question ${questionId} of test ${testId} by user ${user}`
        );
    }
    ...
    return res
      .status(200)
      .send(
        `Answer ${answerId} of test ${testId} by user ${user} pushed to queue. Submited: ${finished}`
      );
  } catch (err) {
    console.error(err);
  }
  ...
  try {
    ...
    if (test.submited) {
      ...
      return res
        .status(200)
        .send(
          `Test ${test.testId} submitted. Submited: ${test.submited} - Score: ${test.score}`
        );
    }
    ...
    return res
      .status(200)
      .send(
        `Answer ${answerId} of test ${testId} by user ${user} awaiting for process. Submited: ${finished}`
      );
  } catch (err) {
    console.error(err);
  }
